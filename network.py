...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        inputs_forward = tf.concat([tf.nn.embedding_lookup(word_embedding, self.input_word),
                                    ...
                                    tf.nn.embedding_lookup(pos2_embedding, self.input_pos2)], axis=2)
        inputs_backward = tf.concat(
            [tf.nn.embedding_lookup(word_embedding, tf.reverse(self.input_word, [1])),
             ...
             tf.nn.embedding_lookup(pos1_embedding,
                                    tf.reverse(self.input_pos2, [1]))], axis=2)
        ...
        output_forward = tf.reshape(tf.concat(outputs_forward, axis=1), [total_num, num_steps, gru_size])
        output_backward = tf.reverse(tf.reshape(tf.concat(outputs_backward, axis=1), [total_num, num_steps, gru_size]),
                                     [1])
